...
class Controller:
    ...
    def process_learner(self, learner: LearnerModel) -> LearnerModel:
        ...
        for topic, data in performance["topics"].items():
            ...
            # Add overall percent_correct for compatibility with pedagogy rules
            total_questions = data["total_questions"]
            total_correct = sum(data.get(f"correct_{diff}", 0) for diff in ["easy", "medium", "hard"])
            data["percent_correct"] = (total_correct / total_questions * 100) if total_questions > 0 else 0.0
